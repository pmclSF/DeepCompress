...
from experiment import assert_exists
from parallel_process import parallel_process, Popen
from pc_metric import validate_opt_metrics
...
def run_experiment(output_dir, model_dir, model_config, pc_name, pcerror_path, pcerror_cfg_path, input_pc, input_norm,
    ...
    additional_params = []
    if fixed_threshold:
        additional_params.append('--fixed_threshold')
    if no_stream_redirection:
        f = None
        additional_params.append('--no_stream_redirection')
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no_stream_redirection', help='Disable stdout and stderr redirection.', action='store_true')
    ...
    keys = [
        ...
        'pcerror_mpeg_mode', 'model_configs', 'opt_metrics', 'max_deltas', 'fixed_threshold'
    ]
    ...
    for experiment in experiments['data']:
        ...
        for model_config in model_configs:
            ...
            for lmbda in lambdas:
                ...
                pcerror_cfg_path = os.path.join(
                    MPEG_TMC13_DIR, 'cfg', pcerror_mpeg_mode, cfg_name, 'r06', 'pcerror.cfg'
                )
                ...
                if not os.path.exists(os.path.join(model_dir, 'done')):
                    ...
                    if not all(os.path.exists(os.path.join(current_output_dir, f'report_{g}.json')) for g in opt_groups):
                        params.append((
                            current_output_dir, model_dir, config, pc_name, PCERROR, pcerror_cfg_path,

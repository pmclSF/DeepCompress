...
def debug_identity_v3(input, device_name="", tensor_name="", io_of_node="", is_input=False, io_index=-1, debug_urls=[], gated_grpc=False, name=None):
  r"""Provides an identity mapping of the non-Ref type input tensor for debugging.
...
def debug_identity_v3_eager_fallback(input, device_name, tensor_name, io_of_node, is_input, io_index, debug_urls, gated_grpc, name, ctx):
  if device_name is None:
    device_name = ""
  device_name = _execute.make_str(device_name, "device_name")
  if tensor_name is None:
